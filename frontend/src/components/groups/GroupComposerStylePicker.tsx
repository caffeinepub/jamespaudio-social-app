...
import { isColorUnlocked, isEffectUnlocked, getRequiredTierForColor, getRequiredTierForEffect, getTierDisplayName } from '../../utils/membershipTier';
...
export default function GroupComposerStylePicker({
  ...
  const handleColorClick = (color: MessageColor) => {
    if (isColorUnlocked(userTier, color)) {
      ...
      if (requiredTier) {
        toast.error(`${getColorLabel(color)} color requires ${getTierDisplayName(requiredTier)} plan`);
      }
    }
  };
  ...
  const handleEffectClick = (effect: MessageEffect) => {
    if (isEffectUnlocked(userTier, effect)) {
      ...
      if (requiredTier) {
        toast.error(`${getEffectLabel(effect)} effect requires ${getTierDisplayName(requiredTier)} plan`);
      }
    }
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Effect Picker */}
      <div>
        <p className="text-xs font-medium text-muted-foreground mb-2">Text Effect</p>
        <div className="flex flex-wrap gap-2">
