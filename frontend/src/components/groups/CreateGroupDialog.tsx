...
interface CreateGroupDialogProps {
  ...
  onSubmit: (name: string, description: string, initialMembers: Principal[]) => void;
...
export default function CreateGroupDialog({
  ...
  const handleRemoveMember = (userId: Principal) => {
    setSelectedMembers(selectedMembers.filter(m => m.toString() !== userId.toString()));
  };
              ...
              {selectedMembers.length > 0 && (
                <div className="flex flex-wrap gap-2 p-2 bg-muted rounded">
                  {selectedMembers.map((memberId) => {
                    const memberResult = searchResults.find(r => r.userId.toString() === memberId.toString());
                    return (
                      <Badge key={memberId.toString()} variant="secondary" className="gap-1">
                  ...
                  <div className="p-2 space-y-2">
                    {searchResults.length > 0 ? (
                      searchResults.map((user) => {
                        const isSelected = selectedMembers.find(m => m.toString() === user.userId.toString());
                        return (
                          <div
                            key={user.userId.toString()}
                            className="flex items-center gap-3 p-2 rounded hover:bg-muted cursor-pointer"
                            onClick={() => !isSelected && handleAddMember(user.userId)}
                          >
