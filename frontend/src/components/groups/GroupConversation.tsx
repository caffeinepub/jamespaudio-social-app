...
import { Send, Video, Smile, UserPlus } from 'lucide-react';
import { useGetGroupMessages, useGetCallerMembershipTier } from '../../hooks/useQueries';
...
import { getColorClasses, renderEffectDecoration, normalizeColor, normalizeEffect } from '../../utils/messageStyle';
...
export default function GroupConversation({ groupId, groupName, memberIds }: GroupConversationProps) {
  ...
  const [messageColor, setMessageColor] = useState<MessageColor>(MessageColor.blue);
  ...
  const handleSendMessage = async () => {
    ...
    setIsSending(true);
    try {
      await actor.sendGroupMessage(groupId, messageText, mediaAttachment, messageColor, messageEffect);
      setMessageText('');
      setMediaAttachment(null);
      // Reset to blue after sending (group default)
      ...
      } else if (error.message?.includes('membership tier')) {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`flex-1 ${isOwnMessage ? 'text-right' : 'text-left'}`}>
                        ...
                        >
                          <p className={`text-sm break-words ${getColorClasses(msgColor, isOwnMessage)}`}>
          ...
          <div className="p-4 border-t bg-blue-50 dark:bg-blue-950/20">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={(!messageText.trim() && !mediaAttachment) || isSending}
              >
                {isSending ? (
