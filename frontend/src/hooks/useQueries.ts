...
import type { UserProfile, Timestamp, Group, GroupMessage, MediaAttachment, GroupId } from '../backend';
import { MessageColor, MessageEffect, MembershipTier } from '../backend';
import type { PublishedApp, PremiumContent, LiveStream, UpcomingFeature, UpcomingGame, FeedItem } from '../types/temporary';
  ...
  return useMutation({
    mutationFn: async ({ username, bio }: { username: string; bio: string }) => {
      ...
      const profile: UserProfile = {
        username,
        bio,
        profilePicture: { url: '/assets/generated/default-avatar.dim_200x200.png', contentType: 'image/png' },
        followers: [],
        following: [],
        status: '',
        points: BigInt(0),
...
export function useSendGroupMessage() {
  ...
  return useMutation({
    mutationFn: async ({ groupId, content, mediaAttachment, color, effect }: { groupId: GroupId; content: string; mediaAttachment: MediaAttachment | null; color: MessageColor; effect: MessageEffect }) => {
      if (!actor) throw new Error('Actor not available');
      return actor.sendGroupMessage(groupId, content, mediaAttachment, color, effect);
    },
    onSuccess: (_, variables) => {
...
export function useSendMessage() {
  ...
  return useMutation({
    mutationFn: async ({ receiver, content, color, effect }: { receiver: Principal; content: string; color: MessageColor; effect: MessageEffect }) => {
      if (!actor) throw new Error('Actor not available');
      return;
    },
    onSuccess: (_, variables) => {
