...
import { useGetUserGroups, useCreateGroup, useAddGroupMember } from '../hooks/useQueries';
...
export default function GroupsPage({ targetGroupId, onGroupSelected }: GroupsPageProps) {
  ...
  const [selectedInitialMembers, setSelectedInitialMembers] = useState<Principal[]>([]);
  ...
  const handleCreateGroup = async (name: string, description: string, initialMembers: Principal[]) => {
    setCreateError('');
    try {
