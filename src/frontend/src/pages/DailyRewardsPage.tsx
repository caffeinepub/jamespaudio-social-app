...
import { Calendar, Coins, Gift, TrendingUp, Zap } from "lucide-react";
...
export default function DailyRewardsPage() {
  ...
  const handleClaimReward = async () => {
    try {
      const _result = await claimReward.mutateAsync();
      ...
      toast.error("Unable to claim reward", {
        description:
          error.message || "You may have already claimed today's reward",
      });
    }
  };
  ...
  const canClaim = userProfile?.lastDailyRewardClaim
    ? Date.now() - Number(userProfile.lastDailyRewardClaim) / 1000000 >
      24 * 60 * 60 * 1000
    : true;
        ...
        <Card className="border-2 bg-gradient-to-br from-card to-[oklch(var(--rewards-accent)/0.1)]">
          <CardContent className="p-8">
            <div className="flex flex-col items-center text-center space-y-6">
              <div
                className={`relative ${showAnimation ? "animate-bounce" : ""}`}
              >
                <img
              ...
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground">
                  Claim your daily reward to earn points and maintain your
                  streak
                </p>
              </div>
